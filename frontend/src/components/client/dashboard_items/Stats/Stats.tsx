...
function Stats() {
  ...
  const { data, isLoading, isError } = useEdzes.getEdzesByType(
    userId,
    filteredValues
  ) as unknown as UseQueryResult<EdzesStatsResponse, Error>;

  const groupedData = groupIzomcsoportCounts(
  ...
  return (
    <div
      ...
      className={clsx(
        styles.mainDiv,
        "sm:max-w-[500px]   max-w-[325px] flex flex-col m-2.5 p-5 rounded-lg "
      )}
    >
      ...
      <div className={styles.flexContainer}>
        <div
          className={clsx(
            styles.humanDiv,
            "max-w-[220px]  max-h-[130px] sm:min-h[130px]  min-w-[130px] w-full sm:ml-0  rounded-lg"
          )}
        >
        ...
        <div
          className={clsx(
            styles.humanDiv,
            "max-w-[220px]  max-h-[130px]  min-w-[130px] w-full sm:ml-0  rounded-lg"
          )}
        >
          <div className={styles.pieChartContainer}>
            <PieChart className={styles["pieChart"]} width={200} height={160}>
              <Pie
                data={groupedData}
                dataKey="value"
                nameKey="name"
