...
export const GyakorlatItem: React.FC<GyakorlatItemProps> = ({ gyakorlat }) => {
  ...
  const content = (
    <>
      ...
      <BodySVG
        size={130}
        selectedMuscleIds={[gyakorlat.fo_izomcsoport]}
        secondaryMuscleIds={gyakorlat.izomcsoportok}
      />
      {!isMobile && (
            ...
            {isOfAdminHeritageUser && (
              <div className={styles.editButton}>
                <Button
                  ...
                  noBackground
                  color="secondary"
                  href={`/gyakorlatok/${gyakorlat.gyakorlat_id}/szerkeszt`}
                ></Button>
              </div>
            )}
          </div>
  ...
  return <div className={styles["gyakorlat-item"]}>{content}</div>;
};
