...
function EdzesekPage() {
  ...
  const gyakorlat_id = searchParamsGyakorlatId
    ? parseInt(searchParamsGyakorlatId)
    : null;
  ...
  if (edzesek?.items[0] && gyakorlat_id !== null) {
    ...
    header =
      "Az alábbi edzések tartalmazzák a keresett gyakorlatot: " + headerHelper;
  }

  return (
    <ContentLayout header={header}>
      <Formik onSubmit={() => {}} initialValues={{ order: filter }}>
      ...
      {edzesek?.items.length === 0 && (
        <Text variant="subtitle-15" className="justify-self-center mb-6">
          Jelenleg még nincsenek kész edzései
        </Text>
      )}
